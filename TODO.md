...
- [x] **issue #6：Intel Mac 装不上（2026-05-15，PR #8 待合并）** —— `build.sh` / `make-dmg.sh` 用裸 `swift build -c release` 默认只编当前架构，Apple Silicon 开发机 → 产物 arm64-only Mach-O → Intel Mac 报 "Bad CPU type in executable" 装不上。两个脚本都加 `--arch arm64 --arch x86_64`，产物路径从 `.build/release/` 改到 `.build/apple/Products/Release/`，输出 universal binary（minos 14.0 兼容 macOS 14）。DMG 体积 1.86MB → 3.2MB，可接受。已在 Heartcoolman fork 开分支 fix/universal-binary-intel-mac，向 upstream 提 PR #8 + 在 issue #6 下贴根因诊断让 bennyzhangdx 跑 `file` 自验。
