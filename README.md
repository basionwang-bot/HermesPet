...
最多同时挂 **3 个对话**（`⌘1` ~ `⌘3` 直达），每个独立绑定 mode 不互相污染。切换对话时 header 的 mode 颜色和图标实时同步，灵动岛精灵也跟着切。
...
### 🦞 双桌宠 · 桌面伴侣（Claude / 在线 AI 模式独占）

切到 Claude 模式空闲 3 分钟 / 切到「在线 AI」立即 → 一个像素小家伙从灵动岛跳到桌面上闲逛：

| 模式 | 桌宠 | 行为差异 |
|---|---|---|
| Claude Code | **Clawd 🦞** 橙色像素螃蟹 | 需要 3 分钟 idle 触发（或开"自由活动"立即出来） |
| 在线 AI | **云朵 ☁️** indigo 像素小精灵 | 切过去立即出来陪伴 |

两种桌宠**共用同一套交互**：

- **会自己嗅文件**（仅 Clawd）：偶尔下到桌面挑一个图标走过去，用 Hermes 给文件名一句 ≤10 字短评（"翻翻这个~" / "好长的名字"），气泡显示
- **会被拖动**：鼠标按住拖到任何桌面图标上松手 → 它在那儿停下嗅 + AI 短评
- **会吃文件**（仅 Clawd）：反过来把文件拖到 Clawd 身上 → 嚼嚼吞下去 → 文件自动作为附件进当前对话 + 发送
- **会蹭鼠标**：鼠标靠近时小跑过来打招呼
- **单击 / 双击**：都是打开聊天窗，不切换 AI 模式（保持用户在当前 mode）
